...
# set seed for recovery of stochastic components, which are:
...
for (p in pheno.indices) {
  ...
  for (i in 1:n.tests) {
    ...
    # remove intercept parameter
    beta.hat.adj <- beta.hat.adj[-1,]
...
# method names and color assignments
col.key <- c("darkgreen","purple","red","yellow","orange","blue","cyan")
leg.key <- c("FoMaC","Multivariate LM","Univariate LM","Combined Univariate LM","PLINK","Combined PLINK","Random Forest")
...
par(font=2)
legend("bottomright",legend=leg.key[method.indices],col=col.key[method.indices],cex=1.0,lty="solid",lwd=5)
par(font=1)
for (midx in method.indices) {
  lines(roc.d[[midx]][,2],roc.d[[midx]][,1],col=col.key[midx])
}

# pr curves
par(font.lab=2)
#YLIM <- sapply(method.indices,function(x) max())
plot(0,0,type="n",xlim=c(0,1),ylim=c(0,1),cex.lab=1.3,xlab="Recall",ylab="Precision")
grid(NULL,NULL)
#plot(0,0,type="n",xlim=c(0,0.1),ylim=c(0,0.6),xlab="False Positive Rate",ylab="True Positive Rate")
par(font=2)
legend("topright",legend=leg.key[method.indices],col=col.key[method.indices],cex=1.0,lty="solid",lwd=5)
par(font=1)
for (midx in method.indices) {
  lines(pr.d[[midx]][,2],pr.d[[midx]][,1],col=col.key[midx])
}

# qq plots
layout(matrix(1:6,2,3,byrow=F))
key <- as.vector(sims[[s]][[3]])
for (midx in 1:6) {
...
layout(matrix(1:6,2,3,byrow=F))
for (midx in 1:6) {
  hist(pvals[,,midx],breaks=10,main=leg.key[midx],xlab="P-value Bins")
}
